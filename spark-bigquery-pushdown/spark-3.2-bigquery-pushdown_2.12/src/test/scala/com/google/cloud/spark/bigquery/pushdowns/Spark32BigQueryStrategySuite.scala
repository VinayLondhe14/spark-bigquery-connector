...
class Spark32BigQueryStrategySuite extends AnyFunSuite with BeforeAndAfter {
  @Mock
  ...
  val expressionConverter: SparkExpressionConverter = new Spark32ExpressionConverter(expressionFactory, sparkPlanFactoryMock)
  ...
  test("generateQueryFromPlanForDataSourceV2 with unsupported node") {
    assert(new Spark32BigQueryStrategy(expressionConverter, expressionFactory, sparkPlanFactoryMock)
  ...
  test("generateQueryFromPlanForDataSourceV2 with unsupported query pushdown scan") {
    ...
    val bigQuerySQLQuery = new Spark32BigQueryStrategy(expressionConverter, expressionFactory, sparkPlanFactoryMock)
